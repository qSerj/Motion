...
- **Модульный монолит**: код разделён на модули с четкими границами, но запускается в одном процессе ради realtime‑производительности.
- **Ports & Adapters**: ядро не зависит от конкретного источника данных (webcam, видеофайл, потенциально VR), а UI/клиенты — адаптеры поверх ядра.
- **Контрактность форматов**: `.mtp v2` и IPC‑схемы считаются ABI; изменения должны быть задокументированы и протестированы.
